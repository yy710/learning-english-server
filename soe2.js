...
const SecretId = credData[0];
const SecretKey = credData[1];

const AUDIO_FOR_ONCE = "./public/audio/bike.mp3";
const AUDIO_FOR_CN = "./public/audio/cn.mp3";
const AUDIO_FOR_SEQ = "./public/audio/Rex was a big dog.wav";
const AUDIO_FOR_ASYNC = "./public/audio/I see sixteen eggs and eighteen apples.wav";
const AUDIO_FOR_SHREK = "./public/audio/111.mp3";
...
async function test() {
    ...
    /**
     *  设置访问域名，如果需要就近部署，可以使用 soe-tencentcloudapi.com,
     *  腾讯云将根据访问的地域解析到合适的服务器上，如果调用服务已确定地域，如华南地区
     *  可以直接使用地域域名，加快访问速度
     */
    ...
    //await EvaluationOnce(client, "Once upon a time there was a lovely princess");
    //await  EvaluationOnce(client, "Rex was a big dog");
    await EvaluationOnce(client, "bike");
 ...
 */
async function EvaluationOnce(client, text) {
    try {
        console.log("-----------一次性评估示例------------");
        ...
        let data = fs.readFileSync(AUDIO_FOR_SHREK);
        //let data = fs.readFileSync(AUDIO_FOR_SHREK);
        ...
        console.log("评估结果：");
        console.log(JSON.stringify(transRsp, null, 4))
...
async function EvaluationMultipleSeq(client) {
    ...
    try {
        ...
        initReq.RefText = "Once upon a time there was a lovely princess But she had an enchantment upon her of a fearful sort";
        // 设置为流式评估
        initReq.WorkMode = 0;
        initReq.EvalMode = 1;
        initReq.ScoreCoeff = 3.5;
...
/**
 *  流式评估可以乱序发送，评估不需要等前一个分片的结果再发送后一个分片，
 *  这样会带来一定的往返延迟，实践中可以将所有音频数据并发发送到腾讯云评估
 */
...
/**
 *  较长音频的评估，短时间内获取不了结果(评估时间超过 20s )，采用异步模式，
 *  主动轮询评估服务得到评估结果，可用于段落或者离线评估
 */
async function EvaluationAsync(client) {
    try {
        ...
        for (let i = 1; i <= pkgNum; i++) {
             ...
             * 三种格式目前仅支持16k采样率16bit编码单声道，如有不一致可能导致评估不准确或失败。
             * @type {number}
             */
            transReq.VoiceFileType = 3;
            ...
            /**
             * 当前数据包数据, 流式模式下数据包大小可以按需设置，
             * 在网络稳定时，分片大小建议设置0.5k，且必须保证分片帧完整（16bit的数据必须保证音频长度为偶数），
             * 编码格式要求为BASE64。
             * @type {string}
             */
            transReq.UserVoiceData = base64Str;
